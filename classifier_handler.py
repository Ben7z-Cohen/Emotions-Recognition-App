...
class ClassifierHandler:
    ...
    def make_training_and_validation_set(self):
        ...
        prediction_data = []
        prediction_labels = []
        for emotion in self.app_engine.EMOTIONS:
            training, prediction = self.get_files(emotion)
            for item in training:
                image = cv2.imread(item)
                gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
                training_data.append(gray)
                training_labels.append(self.app_engine.EMOTIONS.index(emotion))

            for item in prediction:
                image = cv2.imread(item)
                gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
                prediction_data.append(gray)
                prediction_labels.append(self.app_engine.EMOTIONS.index(emotion))
    ...
    def train_and_test_classifier(self):
        ...
        self.fisher_face.save('emotions_recognition.xml')
    ...
    def load_classifier(self):
        self.fisher_face.read('emotions_recognition.xml')
    ...
    def detect(self, gray, frame):
        ...
        for (x, y, w, h) in faces:
            ...
            emotion, c = self.fisher_face.predict(resized_image)
